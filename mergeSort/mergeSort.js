...
// Define a method called mergeSort(array)
// Split the array into 2 halves, left and right
// call merge on the 2 halves
...
console.log(mergeSort([4,7,4,3,9,1]));
...
var subroutine = function() {
    ...
    for (var i = 0; i < array.length; i++) {
      sublists.push([array[i]]);
    }
    
    // Adjacent sublits are merged into soreted sublists
    // Merge step: Adjacent sublists are merged into sorted sublists
    // [[4],[7],[4],[3],[9],[1],[2]] -> [[4,7],[3,4],[1,9],[2]]

    for (var i = 0; i < sublists.length - 1; i++) {
      console.log(sublists[i][0]);
      if (sublists[i][0] > sublists[i + 1][0]) {
        sortedSublists.push(sublists[i+1][0], sublists[i][0])
      } else {
        sortedSublists.push(sublists[i][0], sublists[i+1][0])
      }
    }
  }
  subroutine();
